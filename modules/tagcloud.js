...
export class tagcloud {
	...
	this.h = config.h ? config.h : 400; 
	this.w = config.w ? config.w : 600; 
	this.colorTag = config.colorTag ? config.colorTag : {'select':'red','visit':'green','over':'orange','init':'black'}; 
	...
	this.init = function() {
		...
		function draw(words,wordExt,slideExt=false) {
			...
			var text = vis.selectAll("text")
			    ...
			    .enter().append("text")
		        	...
		        	.attr("cursor", (me.user || me.global || me.verif) ? "pointer" : "zoom-in")
		        	;
		}
		function progress(d) {
		...
		function parseText(start=true,end=true) {
			if(start)me.tags = {};
			var cases = {};
			me.txt.split(me.wordSeparators).forEach(function(word) {
				...
				if (word.length <= 2 && isNaN(word)) return;
		    	if(me.user){
            ...
            if(end){
                let tags = [];
                for (const t in me.tags) {
                    tags.push({'key':t,'vals':me.tags[t],'value':me.tags[t]});
                }
                me.tags = tags;
                me.tags = me.tags.sort(function(a, b) { return b.value - a.value; });
            }
			return me.tags;
		}
		...
		function parseData() {
			...
			gTags.forEach((t)=> {
				...
				if(j>maxTag) return;
                me.txt = t.k;                
                parseText(false, false);            
				j = me.tags.length;
			});
            me.txt = "";                
            parseText(false, true);            
            return me.tags;
		}
